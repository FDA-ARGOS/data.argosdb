...
#        nucleotide=$(echo "$entry" | jq -r '.assembled_genome_acc // "NA"')
        raw_nucleotide=$(echo "$entry" | jq -r '.assembled_genome_acc // "NA"')
        # If assembled_genome_acc contains extra stuff (lineage, description, etc.),
        # take just the first whitespace-separated token (e.g. "MZ629148")
        nucleotide_clean=$(echo "$raw_nucleotide" | awk '{print $1}')

        # And use that as the accession we store & query with
        nucleotide="$nucleotide_clean"
        ...
        # Query the assembly database using eutils API and $GAID which is the assemblyID
         SEARCH_RESULT=$(curl -s "https://eutils.ncbi.nlm.nih.gov/entrez/eutils/esearch.fcgi?db=nucleotide&term=$nucleotide_clean&retmode=json&api_key3$YOURAPIKEY")
#        SEARCH_RESULT=$(curl -s "https://eutils.ncbi.nlm.nih.gov/entrez/eutils/esearch.fcgi?db=assembly&term=$GAID&retmode=json&api_key=$YOURAPIKEY")
        sleep "$sleeptime_wtoken"
        ...
        # Decide what to put in genome_assembly_id column
        # Priority: GCF_ID (from assembly metadata) → GAID (.assembly in JSON) → nucleotide accession
        if [[ -n "$GCF_ID" && "$GCF_ID" != "NA" ]]; then
            genome_assembly_id="$GCF_ID"
        elif [[ -n "$GAID" && "$GAID" != "NA" ]]; then
            genome_assembly_id="$GAID"
        else
            genome_assembly_id="$nucleotide"
        fi

        echo -e "$ORGANISM\t$INFRA_NAME\t$nucleotide\t$genome_assembly_id\t\t\t\t$LINEAGE\t$TAXID\t\tv1.6\t$analysis_platform\t$analysis_platform_object_id\tNCBI\t$OUTPUT\t$CHROMOSOME_COUNT\t$GENES_TOTAL\t$assembly_gc_content\t$length\t$size_gaps\t$size_contigs\t$contig_percentile\t$contig_momentum\t$coverage_contigs\t$cnt_contigs\t$coverage_gaps\t$cnt_gaps\t$gap_percentile\t$genome_coverage\t$n50\t$n75\t$n90\t$n95\t$l50\t$l75\t$l90\t$l95\t$phred_average\t$count_major_mutations\t$count_major_indels\t$mutation_momentum\t$indels_momentum\t$major_mutation_momentum\t$major_indels_momentum\t$alignment_anisotropy\t$overhang_momentum\t$aligned_momentum\t$entropic_momentum\t$reads_unaligned\t$percent_reads_unaligned\t$percent_reads_aligned\t$reads_aligned\t$ASSEMBLY_STATUS\t$rpkm" >> "$output_file"
