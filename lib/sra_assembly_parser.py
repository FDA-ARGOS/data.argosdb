#!/usr/bin/env python3
""" GenBank Assembly Parser

This script will parse the XML result file from a list of Genbank Assemblies. It takes a
single XML file as input, parsing it first as a dictionary, and then
...
def parse_xml( xml_file, samples):
    ...
    for biosample in root:
        ...
        samples[biosample_id] = [sra_id, organism_name, strain, sample_name, \
            ncbi_taxonomy_id, isolate, collected_by, collection_date, \
            geo_loc_name, isolation_source, lat_lon, culture_collection, host,\
            host_age, host_description, host_disease, \
